...
#ifdef SUPPORT_FSK
        case FSK:
            thres = (int8_t)PGM_READ_BYTE(&fsk_cca_thres_table[tal_pib[trx_id].phy.phy_mode.fsk.data_rate]);
...
#ifdef SUPPORT_FSK
        case FSK:
            ack -= (uint8_t)PGM_READ_BYTE(&fsk_processing_delay_ack_timing[tal_pib[trx_id].phy.phy_mode.fsk.data_rate]);
            break;
#endif
#ifdef SUPPORT_OFDM
        case OFDM:
...
#ifdef SUPPORT_FSK
        case FSK:
            rate = 10 * (uint8_t)PGM_READ_BYTE(&fsk_data_rate_table[tal_pib[trx_id].phy.phy_mode.fsk.data_rate]);
            if (tal_pib[trx_id].phy.phy_mode.fsk.mod_type == F4FSK)
            {
                rate *= 2;
            }
