...
#ifdef IQ_RADIO
#    define rf_blk_write(reg, addr, len)    pal_dev_write(RF215_RF, reg, addr, len)
#    define rf_bit_write(reg, val)          pal_dev_bit_write(RF215_RF, reg, val)
#    define rf_reg_write(reg, val)          pal_dev_reg_write(RF215_RF, reg, val)
...
FLASH_DECLARE(AGC_SETTLE_TABLE_TABLE_DATA_TYPE
              agc_settle[AGC_SETTLE_TABLE_ROW_SIZE][AGC_SETTLE_TABLE_COL_SIZE]) = AGC_SETTLE_TABLE;
#endif
#ifdef SUPPORT_FSK
...
retval_t ofdm_rfcfg(ofdm_option_t ofdm_opt, trx_id_t trx_id)
{
    ...
    {
        rx[3] = (0 << AGCS_TGT_SHIFT) | (0x17 << AGCS_GCW_SHIFT); /* AGC target: -21 dB */
    }
    else
    {
...
#ifdef IQ_RADIO
    pal_dev_bit_write(RF215_BB, reg_offset + SR_RF09_TXCUTC_PARAMP, RF_PARAMP32U);   /* PA rampup time */
...
retval_t oqpsk_rfcfg(oqpsk_chip_rate_t chip_rate, trx_id_t trx_id)
{
    ...
    pal_dev_bit_write(RF215_BB, reg_offset + SR_RF09_TXCUTC_PARAMP, RF_PARAMP32U);
#endif /* #ifdef IQ_RADIO */
