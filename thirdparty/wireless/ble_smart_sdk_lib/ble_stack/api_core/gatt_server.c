...
static const uint8_t primary_service_uuid[] = {0x00, 0x28};
static const uint8_t secondary_service_uuid[] = {0x01, 0x28};
static const uint8_t include_uuid[] = {0x02, 0x28};
static const uint8_t char_uuid[] = {0x03, 0x28};

static const uint8_t extended_props_uuid[] = {0x00, 0x29};
static const uint8_t user_desc_uuid[] = {0x01, 0x29};
static const uint8_t client_conf_uuid[] = {0x02, 0x29};
static const uint8_t server_conf_uuid[] = {0x03, 0x29};
static const uint8_t presentation_format_uuid[] = {0x04, 0x29};
...
{	
	...
	gattm_add_attribute_req_handler(service_handle, characteristic->user_desc_len, perm,
		sizeof(user_desc_uuid), (uint8_t *)user_desc_uuid, &characteristic->user_desc_handle);
	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	gattm_add_attribute_req_handler(service_handle, 2, att_permissions(characteristic->server_config_permissions,1,0),
		sizeof(server_conf_uuid), (uint8_t *)server_conf_uuid, &characteristic->server_config_handle);
	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	gattm_add_attribute_req_handler(service_handle, 2, att_permissions(characteristic->client_config_permissions,1,1),
		sizeof(client_conf_uuid), (uint8_t *)client_conf_uuid, &characteristic->client_config_handle);
...
at_ble_status_t at_ble_service_define(at_ble_uuid_t* uuid, at_ble_handle_t* service_handle,
					at_ble_included_service_t * included_list, uint16_t included_count, at_ble_characteristic_gen_t * charactristic_list, uint16_t char_count,at_ble_svc_type svc_type)
{
	...
	do{
		...
		{
			at_ble_characteristic_gen_t *charactaristic = &charactristic_list[i];
			...
			if(is_extended_properties_defined((at_ble_characteristic_t *)&charactristic_list[i]))
			{
				nb_att_uuid_16++;
				total_size += 2;
			}
			if(is_user_desc_defined((at_ble_characteristic_t *)&charactristic_list[i]))
			{
				nb_att_uuid_16++;
				total_size += charactristic_list[i].user_desc_len;
			}
			if(is_server_conf_defined((at_ble_characteristic_t *)&charactristic_list[i]))
			{
				nb_att_uuid_16++;
				total_size += 2;
			}
			if(is_client_conf_defined((at_ble_characteristic_t *)&charactristic_list[i]))
			{
				nb_att_uuid_16++;
				total_size += 2;
			}		
			if(is_presentation_format_defined((at_ble_characteristic_t *)&charactristic_list[i]))
			{
				nb_att_uuid_16++;
				total_size += 7;
			}
			
			if(charactaristic->DescList){
				for(j=0;j<charactaristic->DescCount;j++)
				{
					switch(charactaristic->DescList[j].uuid.type)
					{
					case AT_BLE_UUID_16:
							nb_att_uuid_16++;
						break;
					...
					total_size += charactaristic->DescList[j].desc_val_length;
		...
		*service_handle = start_hdl;
		...
		if(svc_type == PRIMAR_SVC){
			status = gattm_add_attribute_req_handler(start_hdl, uuid_size(uuid), PERM(RD, ENABLE),
				sizeof(primary_service_uuid), (uint8_t *)primary_service_uuid, &temp_handle);
		}else if(svc_type == SECONDARY_SVC){

			status = gattm_add_attribute_req_handler(start_hdl, uuid_size(uuid), PERM(RD, ENABLE),
				sizeof(secondary_service_uuid), (uint8_t *)secondary_service_uuid, &temp_handle);
		}else{
		...
		for(i=0; i < char_count; i++)
		{
			...
			status = gattm_add_attribute_req_handler(start_hdl, charactaristic->value_max_len, char_permissions((at_ble_characteristic_t *)charactaristic),
			...
			temp_buff[0] = char_properties((at_ble_characteristic_t *)charactaristic);
			...
			if(is_extended_properties_defined((at_ble_characteristic_t *)charactaristic))
			{
				extended_properties_define(start_hdl, (at_ble_characteristic_t *)charactaristic);
			}
		
			if(is_user_desc_defined((at_ble_characteristic_t *)charactaristic))
			{
				user_description_define(start_hdl, (at_ble_characteristic_t *)charactaristic);
			}
		
			if(is_server_conf_defined((at_ble_characteristic_t *)charactaristic))
			{
				server_conf_define(start_hdl, (at_ble_characteristic_t *)charactaristic);
			}
		
			if(is_client_conf_defined((at_ble_characteristic_t *)charactaristic))
			{
				client_conf_define(start_hdl, (at_ble_characteristic_t *)charactaristic);
			}
		
			if(is_presentation_format_defined((at_ble_characteristic_t *)charactaristic))
			{
				presentation_format_define(start_hdl,(at_ble_characteristic_t *) charactaristic);
			}

			for(j=0;j<charactaristic->DescCount;j++)
			{
				gattm_add_attribute_req_handler( start_hdl,  charactaristic->DescList[j].desc_val_length, charactaristic->DescList[j].perm, uuid_size(&charactaristic->DescList[j].uuid), charactaristic->DescList[j].uuid.uuid, &charactaristic->DescList[j].handle);
			}
		}
	}while(0);
...
{
	return internal_service_define( uuid, service_handle,included_list,  included_count,  charactristic_list,  char_count,  0);
}
...
at_ble_status_t at_ble_primary_service_define(at_ble_uuid_t* uuid, at_ble_handle_t* service_handle,
					at_ble_included_service_t * included_list, uint16_t included_count, at_ble_characteristic_t * charactristic_list, uint16_t char_count)
{
	return internal_service_define( uuid, service_handle,included_list,  included_count,  charactristic_list,  char_count,  1);
}

at_ble_status_t at_ble_characteristic_value_set(at_ble_handle_t handle,
	uint8_t* value, uint16_t offset, uint16_t len)
{
...
at_ble_status_t at_ble_characteristic_value_get(at_ble_handle_t handle, 
	uint8_t* value, uint16_t offset, uint16_t len, uint16_t actual_read_len)
{
...
at_ble_status_t at_ble_descriptor_value_get(at_ble_handle_t handle, 
	uint8_t* value, uint16_t *len)
{
	uint8_t att_status = gattm_att_get_value_req_handler(handle, len, value);
	at_ble_status_t status = at_ble_att_error(att_status); 
	return status;
}
 ...
 ) 
{
	...
	if(grant_authorization==TRUE)
	{
		uint8_t perm;
		atts_att_get_permission_req_handler(attr_handle, &perm);
		perm &= !(PERM(WR,AUTHZ));
		atts_att_set_permission_req_handler(attr_handle, perm);
	}
	return AT_BLE_SUCCESS;
}
...
#if 0
at_ble_status_t at_ble_descr_value_set(at_ble_handle_t handle,
	uint8_t* value, uint16_t len)
{
	uint8_t att_status = gattm_att_set_value_req_handler(handle, len, value);
	at_ble_status_t status = at_ble_att_error(att_status); 
...
at_ble_status_t at_ble_descr_value_get(at_ble_handle_t handle, 
