	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	if((characteristic->properties | AT_BLE_CHAR_RELIABLE_WRITE) != 0)
	{
		temp_buff[0] |= 0x01;
	}
	if((characteristic->properties | AT_BLE_CHAR_WRITEABLE_AUX) != 0)
	{
		temp_buff[0] |= 0x02;
	}
	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	gattm_add_attribute_req_handler(service_handle, characteristic->user_desc_len, perm,
		sizeof(user_desc_uuid), user_desc_uuid, &temp_handle);
	
	if(characteristic->user_desc != NULL)
	{
		gattm_att_set_value_req_handler(temp_handle, characteristic->user_desc_len, 
	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	gattm_add_attribute_req_handler(service_handle, 2, PERM(RD, ENABLE),
		sizeof(server_conf_uuid), server_conf_uuid, &temp_handle);
	gattm_att_set_value_req_handler(temp_handle, 2, temp_buff);
	                   ...
	                   at_ble_characteristic_t *characteristic)
{
	...
	gattm_add_attribute_req_handler(service_handle, 2, PERM(RD, ENABLE),
		sizeof(client_conf_uuid), client_conf_uuid, &temp_handle);
	gattm_att_set_value_req_handler(temp_handle, 2, temp_buff);
...
static uint16_t char_permissions(at_ble_characteristic_t *characteristic)
{
	...
	if(((characteristic->value_permissions & AT_BLE_ATTR_READABLE_REQ_AUTHN_NO_AUTHR)
		== AT_BLE_ATTR_READABLE_REQ_AUTHN_NO_AUTHR) 
		||((characteristic->value_permissions & AT_BLE_ATTR_READABLE_REQ_AUTHN_REG_AUTHR)
		== AT_BLE_ATTR_READABLE_REQ_AUTHN_REG_AUTHR))
	{
		perm = PERM(RD,AUTH);
	}
	else
	...
	if(((characteristic->value_permissions & AT_BLE_ATTR_WRITABLE_REQ_AUTHN_NO_AUTHR)
		== AT_BLE_ATTR_WRITABLE_REQ_AUTHN_NO_AUTHR)
		||((characteristic->value_permissions & AT_BLE_ATTR_WRITABLE_REQ_AUTHN_REG_AUTHR)
		== AT_BLE_ATTR_WRITABLE_REQ_AUTHN_REG_AUTHR))
	{
		perm |= PERM(WR,AUTH);
	}
	else
	...
	perm |= PERM(IND, ENABLE) | PERM(NTF, ENABLE);
	return perm;
}
...
at_ble_status_t at_ble_primary_service_define(at_ble_uuid_t* uuid, at_ble_handle_t* service_handle,
	...
	gattm_add_svc_req_handler(&start_hdl, total_size,  nb_att_uuid_16,  nb_att_uuid_32,  nb_att_uuid_128);

	/* define "service decl" attribute */
	gattm_add_attribute_req_handler(start_hdl, uuid_size(uuid), PERM(RD, ENABLE),
		sizeof(primary_service_uuid), primary_service_uuid, &temp_handle);
	gattm_att_set_value_req_handler(temp_handle, uuid_size(uuid), uuid->uuid);
	...
	for(i=0; i < included_count; i++)
	{
		...
		gattm_add_attribute_req_handler(start_hdl, uuid_size(&included_list[i].uuid) + 4, PERM(RD, ENABLE),
		...
		gattm_att_set_value_req_handler(temp_handle, uuid_size(&included_list[i].uuid) + 4, temp_buff);
	}
	...
	for(i=0; i < char_count; i++)
	{
		...
		gattm_add_attribute_req_handler(start_hdl, uuid_size(&charactaristic->uuid) + 3, PERM(RD, ENABLE),
			sizeof(char_uuid), char_uuid, &temp_handle);

		/* define the attribute holding the value */
		gattm_add_attribute_req_handler(start_hdl, charactaristic->value_max_len, char_permissions(charactaristic),
		...
		{
			gattm_att_set_value_req_handler(charactaristic->char_val_handle, 
		...
		gattm_att_set_value_req_handler(temp_handle,  uuid_size(&charactaristic->uuid) + 3, temp_buff);
...
{
	gattm_att_set_value_req_handler(handle, len, value);
	return AT_BLE_SUCCESS;
}
...
{
	gattm_att_get_value_req_handler(handle, &actual_read_len, value);
	return AT_BLE_SUCCESS;
}
