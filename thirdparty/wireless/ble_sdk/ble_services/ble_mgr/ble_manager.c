...
at_ble_status_t ble_event_task(void)
{
	if (platform_ble_event_data() == AT_BLE_SUCCESS) {
		...
		AT_BLE_EVENT_TIMEOUT) == AT_BLE_SUCCESS) {
...
static void ble_set_address(at_ble_addr_t *addr)
{
	if (addr == NULL) {
		...
		DBG_LOG(
		"BD Address:0x%02X%02X%02X%02X%02X%02X, Address Type:%d",
		address.addr[5],
		address.addr[4],
		address.addr[3],
		address.addr[2],
...
at_ble_status_t gap_dev_connect(at_ble_addr_t *dev_addr)
{
	...
	return (at_ble_connect(dev_addr, GAP_CONNECT_PEER_COUNT, SCAN_INTERVAL,
	SCAN_WINDOW, &gap_conn_parameter));
}
...
at_ble_status_t gap_dev_scan(void)
{
	...
	return(at_ble_scan_start(SCAN_INTERVAL, SCAN_WINDOW, SCAN_TIMEOUT,
	AT_BLE_SCAN_ACTIVE, AT_BLE_SCAN_GEN_DISCOVERY, false,
	true));
}
...
at_ble_status_t ble_scan_info_handler(at_ble_scan_info_t *scan_param)
{
	if (scan_response_count < MAX_SCAN_DEVICE) {
		...
		memcpy((uint8_t *)&scan_info[scan_response_count++], scan_param,
		sizeof(at_ble_scan_info_t));
...
at_ble_status_t ble_scan_report_handler(at_ble_scan_report_t *scan_report)
{
	if (scan_report->status == AT_BLE_SUCCESS) {
		...
		if (BLE_SCAN_DATA_HANDLER(scan_info,
		scan_response_count) == AT_BLE_FAILURE) {
...
{
	memcpy(ble_connected_dev_info, (uint8_t *)conn_params,
	sizeof(at_ble_connected_t));
	DBG_LOG(
...
{
	DBG_LOG("Device disconnected Reason:0x%02x Handle=0x%x",
	disconnect->reason, disconnect->handle);
}
...
void ble_event_manager(at_ble_events_t events, void *event_params)
{
	switch (events) {
		...
		case AT_BLE_DISCONNECTED:
		{
			...
			(at_ble_disconnected_t *)event_params);
		}
		break;

		case AT_BLE_DISCOVERY_COMPLETE:
		{
			BLE_DISCOVERY_COMPLETE_HANDLER(
			(at_ble_discovery_complete_t *)event_params);
		}
		break;

		case AT_BLE_PRIMARY_SERVICE_FOUND:
		{
			BLE_PRIMARY_SERVICE_FOUND_HANDLER((
			at_ble_primary_service_found_t *)event_params);
		}
		break;

		case AT_BLE_CHARACTERISTIC_FOUND:
		{
			BLE_CHARACTERISTIC_FOUND_HANDLER((at_ble_characteristic_found_t
			*)event_params);
		}
		break;
		...
		{
			BLE_CHARACTERISTIC_READ_RESPONSE((
			at_ble_characteristic_read_response_t *)event_params);
		}
		break;

		case AT_BLE_CHARACTERISTIC_WRITE_RESPONSE:
		{
			BLE_CHARACTERISTIC_WRITE_RESPONSE((
			at_ble_characteristic_write_response_t *)event_params);
		}
		break;
