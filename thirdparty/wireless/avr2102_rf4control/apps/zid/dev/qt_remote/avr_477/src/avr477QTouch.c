    ...
    if(( time_to_measure_touch ) || (burst_flag))         
    {
        ...
        /* If Sensor status is changed, do not sleep, but buzz  */
         ...
         {       
            ...
            else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x02)
            {
                button = KEY_SW09;     
            }
            else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x04)
            {
                button = KEY_SW10;                                
            }
            else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x08)
            {
                button = KEY_SW17;                 
            }
            else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x10)
            {
                button = KEY_SW14;                 
            }
             else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x20)
            {
                button = KEY_SW11;                 
            }
             else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x40)
            {
                button = KEY_SW12;               
            }
             else if (qt_measure_data.qt_touch_status.sensor_states[0] & 0x80)
            {
                button = KEY_SW15;                
            }
            else
            ...
            if(qt_measure_data.qt_touch_status.sensor_states[1] & 0x02)
            {
                button = KEY_SW13; 
                event = BUTTON_PRESSED_EVENT;
#ifdef QDEBUG                
            ...
            if(qt_measure_data.qt_touch_status.sensor_states[1] & 0x01)
            {
#ifdef QDEBUG
               wheel_pos1[wheel_index1++] = qt_measure_data.channel_signals[8]; 
               wheel_pos2[wheel_index2++] = qt_measure_data.channel_signals[9]; 
               wheel_pos3[wheel_index3++] = qt_measure_data.channel_signals[10]; 
               wheel_pos4[wheel_index4++] = qt_measure_data.channel_signals[11]; 
#endif               
               if(qt_measure_data.qt_touch_status.rotor_slider_values[0] <  memory_map.body.wheel_position)
               {
                 button = KEY_ROT_CW;
