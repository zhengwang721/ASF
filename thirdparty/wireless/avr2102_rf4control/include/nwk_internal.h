 ...
 * @brief
 *
 * $Id: nwk_internal.h 33969 2013-01-21 12:58:59Z selvamegala.s $
 *
...
#define nwkcMaxNodeDescListSize     3  /* 3 = (LARGE_BUFFER_SIZE - 3) / sizeof(node_desc_t) */
...
/* The minimum number of entries that must be supported in the node descriptor table
 * generated through the discovery process. */
...
/* The logical channel that was chosen when the RC PAN was formed. (Target device only) */
...
/* The duty cycle of a device in MAC symbols. A value of 0x000000 indicates the device
...
/* The maximum time in MAC symbols, a device shall wait for a response command frame
...
#define EE_OWN_FRAME_CNTR_OFFSET    (EE_NIB_OFFSET + offsetof(struct nib_tag, FrameCounter))
...
#endif
#if (defined RF4CE_SECURITY) || (defined RF4CE_PLATFORM)  /* target and controller */
...
#if (defined RF4CE_TARGET) || (defined CHANNEL_AGILITY) || (defined RF4CE_PLATFORM)
...
#if ((NWK_DISCOVERY_IND_RESP == 1) || (NWK_PAIR_REQ_CONF == 1) || (defined RF4CE_TARGET))
