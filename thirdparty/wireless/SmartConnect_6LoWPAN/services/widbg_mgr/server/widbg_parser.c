...
static void process_incoming_sio_data(void)
{
	...
	case UART_RX_STATE_DOMAIN:
		if (DOMAIN_WIDBG_COMMON == data[rx_index] || DOMAIN_WIDBG_UPGRADE == data[rx_index] || DOMAIN_WIDBG_DEBUG == data[rx_index])
		{
	...
	case UART_RX_STATE_FCS:
		...
		if (sio_fcs == data[rx_index]) {
			...
			handle_widbg_msg(frame_length, sio_rx_buf);
		}
		else
		{
...
void send_pc_data(widbg_domain_t domain_id, uint8_t msg_id, uint8_t *msg, uint16_t len)
{
	uint8_t fcs = 0;
