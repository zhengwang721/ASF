...
typedef enum nwk_enum_tag
{
    ...
    /** A duplicate pairing table entry was detected following the receipt of a
    ...
    /** The security link key exchange or recovery procedure did not complete
     * within the required time. */
    NWK_SECURITY_TIMEOUT        = 0xb9,
    /** A security link key was not successfully established between both ends
     * of a pairing link. */
    NWK_SECURITY_FAILURE        = 0xba,
    /** A parameter in the primitive is either not supported or is out of the
     * valid range. */
    NWK_INVALID_PARAMETER       = 0xe8,
    /** A SET/GET request was issued with the identifier of a NIB attribute that
     * is not supported. */
    NWK_UNSUPPORTED_ATTRIBUTE   = 0xf4,
    /** An attempt to write to a NIB attribute that is in a table failed because
...
typedef enum nib_attribute_tag
{
    ...
    /** The duty cycle of a device in MAC symbols. A value of 0x000000 indicates
     * the device
 ...
 *
 * The NLDE-DATA.request primitive requests the transfer of a data APDU (i.e. NSDU)
 ...
 *
 * The NLME-PAIR.response primitive allows the application to request that the NLME
 ...
 *
 * The NLME-RESET.request primitive allows the application entity to request a reset
 * of the NWK layer.
 *
...
/**
 * @brief NLME-RX-ENABLE.confirm primitive API
 *
 ...
 *
 * The NLME-SET.request primitive allows the application to request the NLME change
 ...
 * IndicationCountThreshold and AutoChannelMode, can be set via NLME-SET.request.
 ...
 *
 * The function nwk_stack_idle() checks the RF4CE network layer and layers
