...
class AVRStudio5Project(GenericProject):
	...
	# List of project types to add icons/images for
	project_types_with_images = ['template']
	...
	def _generate_vstemplate_tree(self):
		...
		if self.project.type in self.project_types_with_images:
	...
	def _generate_description_file_tree(self):
		"""
		Generate project description file element tree --
		this file is used by the AVR Studio 5 example project wizard.
		"""
		def capitalize_first_letter(string):
			return string[:1].upper() + string[1:]

		project_e = ET.Element('Project')

		attrib_to_value = {
			'File'        : os.path.basename(self._get_zip_archive_name()),
			'Name'        : self.project_name,
			'Caption'     : self.project_caption,
			'Description' : self.project_description,
			'Device'      : self._get_selected_device(),
		}

		for attrib, value in attrib_to_value.items():
			project_e.set(attrib, value)

		# Start generating the tree with keywords for the project
		keyword_dict = self._get_project_keywords()

		for category, keywords in keyword_dict.items():
			# Enforce capital, first letter in the category name
			category_name = capitalize_first_letter(category)
			category_e = ET.SubElement(project_e, 'Category')
			category_e.set('Name', category_name)

			for keyword in keywords:
				keyword_e = ET.SubElement(category_e, 'Keyword')
				keyword_e.set('Name', keyword)

		# Add the URL for the help documentation
		help_url = self._get_documentation_help_url(self.project)
		doc_e = ET.SubElement(project_e, 'Documentation')
		doc_e.set('Help', help_url)

		# Add licenses. If any. If not? Go... somewhere else.
		if len(self.project_licenses) > 0:
			licenses_e = ET.SubElement(project_e, 'Licenses')
			for licenses in self.project_licenses:
				for license in self.project_licenses[licenses]:
					lic_e = ET.SubElement(licenses_e, 'License')
					lic_e.text = os.path.join(self.asf_dir, license).replace(os.sep, '\\')
					lic_e.set('Name', licenses.caption)

		# Fix indentation before returning
		asf.helper.indent_xml(project_e)

		return ET.ElementTree(project_e)
	...
	def _get_asf_header_name(self):
		...
		"""
		return os.path.join(self._get_project_output_dir(), "asf.h")
	...
	def _get_framework_config_name(self):
		"""
		Get the filename of the framework configuration file.
		"""
		return self._get_exported_project_file_name() + '.config'
	...
	def _get_description_file_name(self):
		"""
		Get the filename of the XML file to contain the project
		description.
		"""
		return self._get_exported_project_file_name() + '.xml'
	...
	def get_generated_files_list(self):
		...
		"""
		return [
			self._get_vstemplate_name(),
			self._get_avrgccproj_name(),
			self._get_framework_config_name(),
			self._get_zip_archive_name(),
			self._get_description_file_name(),
	...
	def _get_project_data(self):
		...
		# Must add asf.h to the application files
		application_files[BuildHeader].update({self._get_asf_header_name() : None})
	...
	def write(self):
		...
		avrgccproj_tree = self._generate_avrgccproj_tree()
		description_file_tree = self._generate_description_file_tree()
		asf_header_content = self._generate_asf_header_content()
		...
		avrgccproj_name = self._get_avrgccproj_name()
		zip_archive_name = self._get_zip_archive_name()
		description_file_name = self._get_description_file_name()
		asf_header_name = self._get_asf_header_name()

		vstemplate_tree.write(vstemplate_name)
		avrgccproj_tree.write(avrgccproj_name)
		description_file_tree.write(description_file_name)
		self._write_file(asf_header_name, asf_header_content)

		# -- Zip the project --
		if not self.runtime.no_zip:
			zip_archive = ZipFile(zip_archive_name, 'w', ZIP_DEFLATED)
			zip_archive.write(vstemplate_name, self.project_name + '.vstemplate')
			zip_archive.write(avrgccproj_name, self.project_name + '.cproj')
		...
		if self.project.type in self.project_types_with_images:
			...
			if vstemplate_icon:
				if not self.runtime.no_zip:
					# ZIP it
					zip_archive.write(vstemplate_icon, self.icon_name)
				else:
					# Copy it
					shutil.copy(vstemplate_icon, os.path.join(self._get_project_output_dir(), self.icon_name))

			vstemplate_image = self._get_vstemplate_image_path()
			if vstemplate_image:
				if not self.runtime.no_zip:
					# ZIP it
					zip_archive.write(vstemplate_image, self.image_name)
				else:
					# Copy it
					shutil.copy(vstemplate_image, os.path.join(self._get_project_output_dir(), self.image_name))

		# Copy all the project's files
		for file_source, file_target in self.project_compiler_files_dict.items() + self.project_distribute_files_dict.items():
			if not self.runtime.no_zip:
				# Copy to ZIP archive
				zip_archive.write(file_source, file_target)
			else:
		...
		self.project.filelist.add(description_file_name)
