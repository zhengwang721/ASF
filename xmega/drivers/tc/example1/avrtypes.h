 ...
 *
 * $Id: avrtypes.h 28857 2011-10-11 09:26:47Z awachtle $
 *
...
/* PAL_GENERIC_TYPE is not known to avrtypes.h, so this
 * #if is disabled, because it breaks cold.
 *
...
#define WDT_SYSTEM_RESET_MODE(x) \
    do \
    {\
        MCUSR = 0;\
        MCUSR &= ~(1 << WDRF);\
...
#define WDT_INT_AND_SYS_RST_MODE(x) \
    do \
    {\
        MCUSR &= ~(1 << WDRF);\
        WDTCSR = (1 << WDCE) | (1 << WDE);\
